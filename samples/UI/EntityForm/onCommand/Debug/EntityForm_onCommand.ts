...
/** Handle on command click and create associated task with open status, if record is new.*/
function onCommandCreateTask() {
    MobileCRM.UI.EntityForm.onCommand("custom_createTask", (entityForm) => {
        if (!entityForm.entity.isNew) {
            let props = entityForm.entity.properties;
            let task = MobileCRM.DynamicEntity.createNew("task");
